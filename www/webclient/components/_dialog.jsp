...
<%
	...
	<%	}	%>
			<table id="<%=id%>_inner" <%=innerAutoId%> positionAtTop="<%=positionAtTop%>" positionX="<%=positionX%>" positionY="<%=positionY%>" modal="<%=modal%>" width="<%=width%>" height="<%=height%>" class="dh mc<%if(modal){%> modal<%}%>" border="0" defaultbutton="<%=defaultbutton%>" control="true" <%=control_events%> style="visibility:hidden;" >
                <tr>
	                <td colspan="3">
	...
	else
	{
		...
		if ( !component.skipRender() )
		{
        	...
        	%><%=finalScriptStart%>fixDialogPosition('"+id+"','"+control.getId()+"',"+max+");<%=finalScriptEnd%><%
        }
	}	%>
	<%=finalScriptStart%>if(dojo.byId('<%=id%>_inner')){var <%=component.getSafeId()%>DnD = new dojo.dnd.Moveable(dojo.byId('<%=id%>_inner'),{skip: true, handle: dojo.byId('<%=id%>_content0')});}<%=finalScriptEnd%>
