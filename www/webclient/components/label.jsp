...
if(componentVisible && isVisible.equals("true"))
{
	...
	if(!image.equals(""))
	{
		...
		String imgAlt = (accessibilityMode)? "" : tooltip;
		image = "<img id='"+id+"_image' src='"+imageSrc+"' "+imagecss+" border='0' alt=\""+imgAlt+"\" style='"+imageVerticalAlign+";margin:0px;margin-"+alignment.begin+":3px;margin-"+alignment.end+":3px;'/>";
	}
	String ul= "";
	...
	else if(!mxevent.equals("") && !isMasked)
	{
		...
		{
			href = " noclick='1' title='"+WebClientRuntime.removeQuotes(HTML.decode(tooltip))+"' onfocus='input_onfocus(event,this);' href='Javascript: setCurrentfocusFromId(event,\""+id+"\"); setClickPosition(document.getElementById(\""+id+"\"), event); sendEvent(\""+mxevent+"\",\""+id+"\",\""+eventValue+"\");' onclick='' ";
			elementType = "a";
		}
		else
			elementType = "span";
	...
	{
		text = WebClientRuntime.makesafevalue(text);
	}
	else
	{
	...
	// PP: This must come AFTER makesafevalue, or <br> will be displayed literally if htmltags is false
	...
	if(((!text.trim().equals("") && !text.equals("&nbsp;")) || showblank.equalsIgnoreCase("true")) || !ri_image.equals("") || !image.equals(""))
	{
		if(accessibilityMode)
		{
			...
			if(!WebClientRuntime.isNull(accessiblelabelkey))
			{
				...
				if(htmltags || component.isOnTableRow())
				{
					tooltip = WebClientRuntime.makesafevalue(text);
				}
				else
				{
					tooltip = text;
		...
		if((controlType.equals("table-title") || controlType.equals("table-text")) && rownum!=null && !rownum.equals("") || controlType.equals("longdescription"))
		{
			...
			{
				if(text.length() > Integer.parseInt(wraplength))
		...
		if(!hideWhenEmpty || !text.trim().equals("") || !text.trim().equals("&nbsp;"))
		{
			...
			if(controlTitleIndex >= 0)
			{
				...
				if(fieldValue.equals(""))
					fieldValue = "__";
				tooltip=WebClientRuntime.replaceString(tooltip, "{control}", WebClientRuntime.makesafevalue(fieldValue));
			}
...
<%					if(dataType == MXFormat.DATE)
					{
						dojoDatePattern += "datePattern:'" + MXFormat.getDatePattern(userLocale) + "',selector:'date'";
					}
					else if(dataType == MXFormat.DATETIME)
					{
			...
			if(heading)
			{
				%><h1 labelledby="<%=id%>" style="display:inline"<%
					if(!accessibilityMode)
					{
						%> title="<%=tooltip%>"<%
					}
			...
			%><<%=elementType%> <%if(verticalLabels){%><%=align%> <%}%>ctype="<%=cType%>"<%
					%> <%=disabled%> id="<%=id%>" <%=ul%> <%=href%> <%=designerEnabled%> <%=noclick%><%
					%> <%=eventValue%> <%=targetId%> <%=ariaRole%> <%=mxevent%> <%=accesskey%> <%=cssclass%><%
					%> style="<%=display%><%=padding%><%=lineheight%>;<%=makeLink%>;<%=colorStr%>;<%=fontSize%>;<%=topMargin%>"<%
					...
					if(!accessibilityMode || text.length() == 0)
					{
						%> title="<%=WebClientRuntime.removeQuotes(HTML.decode(tooltip))%>"<%
					}
					if(isDate&&isNational)
					{
						%><%=dojoTypeInput%><%
